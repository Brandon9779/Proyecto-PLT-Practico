...
class Parser(object):
    ...
    def expect(self, token_type):
        if self.current_token is None:
            ...
            raise ParserError
        if not (self.current_token.type == 'ID' or self.current_token.type.isupper()):
            if self.current_token.type not in ('SERPIENTE', 'COMIDA', 'PARED', 'CABEZA', 'FONDO'):
                self.errors.append(
                    f"❌ Error sintáctico en línea {self.current_token.line}, col {self.current_token.col}: "
                    f"se esperaba 'ID' pero se encontró '{self.current_token.lexeme}'"
                )
                raise ParserError
        self.advance()
...
if __name__ == '__main__':
    ...
    if ast:
        ...
        ast_json = json.dumps(ast.to_dict(), indent=2)
        print(ast_json)

        with open("arbol.ast", "w", encoding="utf-8") as f:
            f.write(ast_json)

        print("\n✅ Árbol sintáctico guardado en 'arbol.ast'")
